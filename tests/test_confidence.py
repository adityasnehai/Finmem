import pytest
from finmem.reasoning.confidence import confidence_gate, HIGH_THRESHOLD, LOW_THRESHOLD
from finmem.data.schemas import QueryResult, MarketState
...
def _make_result(confidence: float) -> QueryResult:
    ...
    return QueryResult(
        ...
        has_analog=confidence >= HIGH_THRESHOLD,
    )


def test_high_confidence_passes():
    result = _make_result(HIGH_THRESHOLD + 0.10)
...
def test_medium_confidence_passes_with_warning():
    result = _make_result((HIGH_THRESHOLD + LOW_THRESHOLD) / 2)
    should_reason, prefix = confidence_gate(result)
    assert should_reason is True
...
def test_low_confidence_refuses():
    result = _make_result(LOW_THRESHOLD - 0.05)
...
def test_exact_high_threshold():
    result = _make_result(HIGH_THRESHOLD)
    should_reason, prefix = confidence_gate(result)
    assert should_reason is True
    assert prefix == ""
...
def test_exact_low_threshold():
    result = _make_result(LOW_THRESHOLD)
